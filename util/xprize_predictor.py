# Modification of the provided standard predictor
...
class XPrizePredictor(object):
    ...
    def __init__(self, path_to_model_weights, df):
        ...
        self.df = df#self._prepare_dataframe(data_url)
    ...
    def predict(self,
        ...
        npis_df = ips_df#self._load_original_data(path_to_ips_file)
        ...
        for g in geos:
            ...
            if len(cdf) == 0:
                ...
                pred_new_cases, ratios = self._get_new_cases_preds(cdf, g, npis_gdf, country_samples)
            ...
            for i, pred in enumerate(zip(pred_new_cases, ratios)):
                cases, ratio = pred
                ...
                forecast["PredictedDailyNewCases"].append(cases)
                forecast["PredictionRatio"].append(ratio)
    
        forecast_df = pd.DataFrame.from_dict(forecast)
        # return all predictions up to the end date
        if len(forecast_df) == 0:
            return forecast_df
        
        # include a few more values so we can fill for geoids that are missing
        # day(s) of data at the end
        return forecast_df[(forecast_df.Date <= end_date)]
